...
def check_once(robot_ip: str, robot_port: int, registers: list[int]) -> bool:
    ...
    try:
        ...
        print(f"[{timestamp()}] OK connected to {robot_ip}:{robot_port} | {values}", flush=True)
        return True
    except Exception as error:
        print(f"[{timestamp()}] FAIL {robot_ip}:{robot_port} | {error}", file=sys.stderr, flush=True)
        return False
    finally:
